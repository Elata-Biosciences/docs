...
<Note>
  These ProtocolConfig splits are used by the FeeManager epoch settlement path. The FeeSwapper uses a simpler 80/20 contributor/treasury split. See [Fee Pipeline](/builders/fee-pipeline) for details on which path is active.
</Note>
