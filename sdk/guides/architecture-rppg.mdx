...
| --- | --- | --- |
| Web | Rust to WASM via `rppg-wasm`, wrapped by `rppg-web` for a stable TS API | `@elata-biosciences/rppg-web` |
| Native | Rust to FFI via `rppg-ffi` | `rppg-ffi` crate |
| Compatibility | Constructors also available through `eeg-wasm` / `eeg-ffi` | `@elata-biosciences/eeg-web` |
